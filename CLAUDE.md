...
## 카탈로그 가격
- VAT 포함가 기준 (사용자 실제 청구액)
- 웹 결제가 기준 (인앱은 스토어 수수료 포함이라 제외)
- 원화 청구로 전환된 서비스는 KRW로
- 종료 서비스는 카탈로그에서 제거, 기존 등록 데이터는 유지
- 검증일자를 catalog.ts 상단 주석에 기록
...
## 다음
1. 카탈로그 가격 반영 — 원칙은 위 "카탈로그 가격" 절, 대상은 notes/catalog-price-audit.md 1·2절
   - 적용 예: 음원 3사 VAT 포함가로 · 카카오 이모티콘 웹가 3,900 · ChatGPT는 KRW(Plus ₩29,000)로 전환, Claude는 USD 유지($20, VAT는 결제 시 가산) · 신세계 유니버스·Notion AI·네이버웹툰 쿠키 제거
   - 제거 항목의 기존 행: findCatalogItem이 null → 직접 입력 취급되는지 확인 (v5 스냅샷은 마이그레이션 전용이라 표시엔 안 쓰임)
